...
class RobertaNER(nn.Module):
  """
  Implement NN class based on roberta pretrained from Hugging face.
  Inputs : 
  ...
  """
    ...
    if labels == None:
      out = self.pretrained(input_ids = input_ids, attention_mask = attention_mask ) #if token is PAD or not
      return out
...
model= RobertaNER(len(tag_list))
model.load_state_dict(torch.load('Roberta-NER_simple.pt'))
model.eval()